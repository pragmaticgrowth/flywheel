...
All tools are MCP tools accessed via `mcp__mcp-droid__<tool_name>`. Every tool inherits the caller's cwd by default and accepts an optional `cwd` parameter.
...
```typescript
mcp__mcp-droid__do_silent_scan({
...
```typescript
mcp__mcp-droid__do_session_continue({
  session_id: "abc-123",
...
```typescript
mcp__mcp-droid__do_session_list({
...
```typescript
mcp__mcp-droid__do_list_models({})
```
...
```typescript
mcp__mcp-droid__do_list_profiles({})
```
