...
export function parseAuditJson(text: string): AuditStructured {
  const empty: AuditStructured = {
    blockers: [],
    concerns: [],
    missed_requirements: [],
    strengths: [],
  ...
  if (!m) return empty;
  try {
    const parsed = JSON.parse(m[1]) as Partial<AuditStructured>;
...
export function stripAuditJsonBlock(text: string): string {
    ...
    "do_audit",
    {
      ...
      inputSchema: {
        context: z
          .string()
          .describe(
            "Original plan, acceptance criteria, or what was requested. Baseline for the audit.",
          ),
        diff: z
          .string()
          .optional()
        ...
        thread_id: z
          ...
          .optional()
          .describe(
            "Existing Codex thread (e.g. from a prior do_discuss). If given, Codex audits with full prior conversation context.",
          ),
        model: z
          .string()
          .optional()
