...
const PRESETS: PresetSpec[] = [
  {
    name: "do_review",
    description:
      "Code review for bugs, security, and edge cases. Returns severity-rated findings with file:line citations. Skeptical by default — only reports material issues, not style.",
  ...
  },
  {
    name: "do_architect",
    description:
      "Architecture analysis — evaluates structure, identifies risks, and recommends improvements with explicit trade-off assessments. Uses the deepest analysis model.",
    ...
    try {
      ...
      if (provider === "droid" && !(await profileExists(spec.droid.profile_file))) {
        return createErrorResponse(
          `droid profile not found at ${spec.droid.profile_file}`,
        );
      }
      ...
      const result = await runWithProvider(provider, {
        ...
        tags: input.tags,
        system_prompt_file:
          provider === "droid" ? spec.droid.profile_file : undefined,
        // Opencode-specific
