    ...
    "do_research",
    {
      ...
      inputSchema: {
        ...
        depth: z
          ...
          .optional()
          .describe(
            "Research depth. `deep` (default) — thorough structured report. `fast` — concise <200-word answer for quick lookups.",
          ),
      },
    },
    async (input): Promise<McpToolResponse> => {
      try {
        ...
        const result = await runWithProvider(provider, {
          ...
          tags: input.tags,
          system_prompt_file:
            provider === "droid" ? RESEARCHER_PROFILE : undefined,
