...
export function spawnOpencodeRun(
...
export function cleanOpencodeOutput(raw: string): string {
  ...
  for (const rawLine of lines) {
    ...
    if (/^> [\w-]+ · /.test(line)) continue; // "> review · glm-5-turbo"
    if (/^→ (Read|Glob|Grep|Task|WebFetch|WebSearch|TodoWrite|LSP) /.test(line)) continue;
    if (/^← (Read|Write|Edit|Glob|Grep|Task|WebFetch|WebSearch|LSP) /.test(line)) continue;
    if (/^✱ (Grep|Glob|Read|WebSearch) /.test(line)) continue;
    if (/^⚙ /.test(line)) continue; // tool execution marker
    if (/^% (WebFetch|WebSearch) /.test(line)) continue;
    if (/^✗ (Error|Failed|Timeout)/.test(line)) continue;
    if (/^\s*opencode run /.test(line)) continue; // `time` trailing echo
    kept.push(line);
  }
  return kept.join("\n").trim();
}
