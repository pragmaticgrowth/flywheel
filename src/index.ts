...
import { loadConfig, resolveProvider, resolveModel, DEFAULT_MODELS, DEEP_MODELS, FAST_MODELS, CROSS_REVIEW_MODELS, PR_REVIEW_MODELS, labelFor, type ProviderName } from "./config.js";
...
interface PresetDef {
  description: string;
  promptBuilder: (userPrompt: string) => string;
  defaultModel: Record<ProviderName, string>;
  droidProfile?: string;
  opencodeAgent?: string;
  autoLevel?: string;
}

function makePreset(def: PresetDef) {
  return tool({
    ...
    async execute(args, ctx) {
      ...
      if (!result.ok) throw new Error(result.error_message || "tool failed");
      return result.text;
    },
  });
}
    ...
    tool: {
      ...
      do_cross_review: tool({
        ...
        async execute(args, ctx) {
          ...
          const sections: string[] = [`# Cross-Model Review [${provider}]\n`];
      ...
      do_pr_review: tool({
        ...
        async execute(args, ctx) {
          ...
          if (!isGitRepo) throw new Error(`not a git repository: ${cwd}`);

          let base = args.base;
          if (!base) {
            for (const candidate of ["main", "master", "develop"]) {
              const { ok } = await git(["rev-parse", "--verify", candidate], cwd);
              if (ok) { base = candidate; break; }
            }
            if (!base) throw new Error("could not detect base branch");
          }
          ...
          if (!diffR.stdout) return `No changes to review (${branchR.stdout || "HEAD"} is up to date with ${base}).`;

          let diff = diffR.stdout;
          const MAX = 80_000;
          const truncated = Buffer.byteLength(diff) > MAX;
          if (truncated) diff = Buffer.from(diff).subarray(0, MAX).toString("utf8");
          ...
          if (!result.ok) throw new Error(result.error_message || "PR review failed");
      ...
      do_exec: tool({
        ...
        async execute(args, ctx) {
          ...
          if (!result.ok) throw new Error(result.error_message || "exec failed");
          return result.text;
        },
      }),
      ...
      do_session_continue: tool({
        ...
        async execute(args, ctx) {
          ...
          if (!result.ok) throw new Error(result.stderr || "session continue failed");
