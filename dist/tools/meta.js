...
import { createJsonResponse, createUnexpectedErrorResponse, } from "../utils/errors.js";
export function registerMetaTools(server) {
    server.registerTool("do_list_models", {
        description: "List custom (BYOK) models from ~/.factory/settings.json. Returns canonical id, short alias, display name, and provider.",
        inputSchema: {},
    }, async () => {
        try {
    ...
    server.registerTool("do_list_profiles", {
        description: "List droid agent profiles from ~/.factory/droids/*.md (global) and <cwd>/.factory/droids/*.md (project-local). Project profiles shadow global ones.",
        inputSchema: {
