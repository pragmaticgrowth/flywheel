...
import { buildDroidExecArgs } from "../droid/flags.js";
import { getMissionProgress, getMissionStatus, killProcessGracefully, listMissions, markMissionState, missionStateFile, pollForNewMissionDir, resolveMissionDir, } from "../droid/missions.js";
import { TagSpecSchema } from "../schemas/exec.js";
import { resolveCwd } from "../utils/cwd.js";
import { createErrorResponse, createJsonResponse, createUnexpectedErrorResponse, } from "../utils/errors.js";
export function registerMissionTools(server) {
    server.registerTool("droid_mission_start", {
        description: "Start a new droid mission — spawns `droid exec --mission` as a DETACHED background process and polls ~/.factory/missions/ for the new directory. Returns as soon as the directory appears (typically 10–30 seconds), leaving the mission running independently. The mission directory is recognized by its working_directory.txt file (which droid writes BEFORE state.json — state.json may not exist for many seconds, sometimes never). IMPORTANT: droid only triggers real mission orchestration for SUBSTANTIAL prompts (multi-feature work, refactors, implementations). Trivial prompts ('say hi') complete as a plain exec and never create a mission directory — the tool detects this and returns mission_triggered=false. Poll progress later with droid_mission_status / droid_mission_progress using the returned uuid.",
        inputSchema: {
            prompt: z.string(),
            ...
            allow_unsafe: z
                .boolean()
                .optional()
                .describe("Use --skip-permissions-unsafe instead of --auto high. Only in isolated environments."),
            ...
            timeout_ms: z
                ...
                .positive()
                .optional()
                .describe("How long to wait for the new mission directory to appear before giving up. The mission keeps running in the background after this returns either way. Default 120 seconds."),
        },
    }, async ({ prompt, cwd, model, allow_unsafe, tags, timeout_ms, }) => {
        try {
            ...
            const beforeUuids = new Set((await listMissions({ all: true, limit: 100_000 })).map((m) => m.uuid));
            ...
            const logPath = joinPath(tmpdir(), `mcp-droid-mission-${Date.now()}.log`);
            ...
            const stateJsonExists = !(status?.mission_id?.startsWith("pending-") ?? true);
            return createJsonResponse({
    ...
    server.registerTool("droid_mission_list", {
        description: "List droid missions from ~/.factory/missions/, filtered by cwd by default (pass all=true for every mission). Returns mission_id, uuid, state, completed/total features, created/updated timestamps, sorted by updatedAt desc.",
        inputSchema: {
            cwd: z.string().optional(),
    ...
    server.registerTool("droid_mission_status", {
        description: "Get full status for a mission (accepts either mis_xxx or the directory uuid). Returns state, completed/total features, current worker session, and recent progress events. Handoffs are summarized by default — set include_handoffs=true to include the full (potentially 10KB+) payload.",
        inputSchema: {
            mission_id: z.string(),
    ...
    }, async ({ mission_id, include_progress, progress_limit, include_handoffs, include_features, }) => {
        try {
            ...
            if (!status) {
                return createErrorResponse(`mission not found: ${mission_id}. Use droid_mission_list to see available missions.`);
            }
    ...
    server.registerTool("droid_mission_cancel", {
        ...
        inputSchema: {
            ...
            droid_pid: z
                ...
                .positive()
                .optional()
                .describe("The orchestrator pid returned by droid_mission_start. Optional but strongly recommended — without it we can only kill the currently-running worker, not the orchestrator that will spawn the next one."),
            force: z
                .boolean()
                .optional()
                .describe("If true, skip SIGTERM and go straight to SIGKILL. Use for truly stuck processes that ignore SIGTERM. Default false."),
            write_state: z
                .boolean()
                .optional()
                .describe("Write state.json with state='cancelled' after killing processes. Default true. Set to false if you want to preserve the raw state for investigation."),
        },
    }, async ({ mission_id, droid_pid, force, write_state, }) => {
        try {
            ...
            const killResults = await Promise.all(pidsToKill.map(async ({ pid, role }) => {
            ...
            if (pidsToKill.length === 0) {
                warnings.push("no pids to kill: droid_pid not provided and state.json had no currentWorkerPid. Mission may still be running under factoryd — check `pgrep -f 'droid exec --mission'`.");
            }
            if (write_state !== false && !stateUpdated) {
                warnings.push("failed to write state.json with state='cancelled'. The mission may not show as cancelled in droid_mission_list until something else updates it.");
            }
    ...
    server.registerTool("droid_mission_progress", {
        description: "Poll progress events for a mission. Returns a paginated slice of events from progress_log.jsonl. Pass since_offset or since_timestamp from the previous response to get only new events.",
        inputSchema: {
            mission_id: z.string(),
    ...
    }, async ({ mission_id, since_offset, since_timestamp, limit, event_types, exclude_handoffs, }) => {
        try {
