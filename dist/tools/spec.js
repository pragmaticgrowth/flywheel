...
export function registerSpecTool(server) {
    server.registerTool("droid_spec", {
        ...
        inputSchema: {
            ...
            auto: AutoLevelSchema.optional().describe("Autonomy level. Defaults to 'low' to prevent spurious blocked-tool exits. Pass 'high' for real work or null to force read-only (not recommended for spec)."),
            timeout_ms: z.number().int().positive().optional(),
        },
    }, async ({ prompt, cwd, spec_model, spec_reasoning_effort, model, auto, timeout_ms, }) => {
        try {
