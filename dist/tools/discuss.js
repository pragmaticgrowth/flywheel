...
export function registerDiscussTool(server) {
    server.registerTool("do_discuss", {
        description: "Discuss a plan, architecture, or approach with GPT-5.4 xHigh via a persistent Codex MCP backend. Sounding board for design decisions — returns a structured critique (objective, risks, blockers, alternatives, missing, verdict) plus the full markdown response. Pass thread_id to continue an existing discussion (follow-ups are ~10x faster than first turn). Read-only sandbox; does not write code.",
        inputSchema: {
            prompt: z
                .string()
                .describe("The plan, proposal, diff, or question to discuss. Include context — acceptance criteria, constraints, prior decisions."),
            thread_id: z
                .string()
                .optional()
