 ...
 */
import { z } from "zod";
import { AutoLevelSchema, ReasoningEffortSchema, TagSpecSchema, } from "./exec.js";
export const ProviderSchema = z
    .enum(["droid", "opencode"])
    .optional()
    .describe("Execution backend. Defaults to the server's configured default (DO_DEFAULT_PROVIDER env or ~/.config/mcp-do/config.json).");
export const PresetInputShape = {
    ...
    model: z
        .string()
        .optional()
        .describe("Override model. Accepts short aliases (glm-5-turbo, gpt-5.4, gpt-5.4-high) or provider-specific IDs."),
    auto: AutoLevelSchema.optional().describe("Autonomy level (droid only). low / medium / high."),
