...
export async function listSessions(opts = {}) {
    ...
    const narrowToCwd = opts.scan_disk && !opts.all && opts.cwd !== undefined
        ? opts.cwd
        : undefined;
    const collected = opts.scan_disk
        ? await listSessionsFromDisk(opts.sessions_dir, narrowToCwd)
        : await listSessionsFromIndex(opts.index_path ?? join(homedir(), ".factory", "sessions-index.json"));
 ...
 */
export function encodeCwdToSessionsDir(absCwd) {
    return absCwd.replace(/^\//, "-").replace(/\//g, "-");
}
async function listSessionsFromIndex(indexPath) {
    let raw;
    try {
 ...
 */
async function listSessionsFromDisk(sessionsDir, narrowToCwd) {
    ...
    const sessions = await Promise.all(allFiles.map(async ({ jsonlPath }) => {
 ...
 */
export async function readSessionMetaFromJsonl(jsonlPath) {
