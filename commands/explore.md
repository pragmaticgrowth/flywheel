...
allowed-tools: mcp__mcp-do__do_explore
---

Forward a codebase exploration question to the mcp-do explore tool. The headless model reads files and traces call chains.

**Question:** `$ARGUMENTS`

If `$ARGUMENTS` is empty, ask the user what they want to find.

Call `mcp__mcp-do__do_explore` with prompt set to `$ARGUMENTS`.
